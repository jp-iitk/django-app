...
urlpatterns = [
    ...
    auth_views.PasswordResetConfirmView.as_view(template_name='password_reset_confirmation.html'),
    ...
    re_path(r'^boards/(?P<pk>\d+)/topics/(?P<topic_pk>\d+)/$', views.TopicPostListView.as_view(), name='topic_posts'),
    ...
    re_path(r'^boards/(?P<pk>\d+)/topics/(?P<topic_pk>\d+)/posts/(?P<post_pk>\d+)/delete/$',views.PostDeleteView.as_view(), name='delete_post'),
    path('profile/', accounts_view.user_profile, name='user_profile'),
    path('settings/account',accounts_view.UserUpdateView.as_view(), name='user_account'),
